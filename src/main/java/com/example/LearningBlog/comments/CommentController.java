...
public class CommentController {
    ...
    public List<CommentDto> getComments() {
        return commentService.getComments().stream().map(commentDtoMapper::mapCommentToCommentDto).collect(Collectors.toList());
    }
    ...
    public ResponseEntity<List<CommentDto>> getPostComments(@PathVariable("postId") Long postId) {
        ...
        return ResponseEntity.ok(postComments.stream().map(commentDtoMapper::mapCommentToCommentDto).collect(Collectors.toList()));
    }
}
